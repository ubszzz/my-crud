...
class UsersList extends Component {
  ...
  fetchUsers = () => {
    fetch("http://mgt2.pnu.ac.th/6260704014/php-react/all-users.php")
      .then((response) => {
        response.json().then(
          function (data) {
            if (data.success === 1) {
  ...
  handleUpdate = (id) => {
    Axios.post("http://mgt2.pnu.ac.th/6260704014/php-react/update-user.php", {
      id: id,
      user_name: this.name.value,
      user_email: this.email.value,
      user_gender: this.gender.value,
    })
      .then(({ data }) => {
        if (data.success === 1) {
          let users = this.state.users.map((user) => {
            if (user.id === id) {
              ...
              user.user_gender = this.gender.value;
  ...
  handleDelete = (id) => {
    ...
    Axios.post("http://mgt2.pnu.ac.th/6260704014/php-react/delete-user.php", {
      id: id,
    })
      .then(({ data }) => {
        if (data.success === 1) {
  ...
  render() {
    let allUsers = this.state.users.map(
      ({ id, user_name, user_email,user_gender, isEditing }, index) => {
